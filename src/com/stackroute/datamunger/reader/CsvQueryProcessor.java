...
public class CsvQueryProcessor implements QueryProcessingEngine {
		...
		while ((line = bufferedReader.readLine()) != null) {
			...
			if (queryParameter.getRestrictions() == null)
				...
				for (int i = 0; i < queryParameter.getRestrictions().size(); i++) {
					...
					booleanArrayList.add(filter.evaluateExpression(queryParameter.getRestrictions().get(i), rowFields[index].trim(), rowDataTypeDefinitionMap.get(index)));
				}
				result = solveOperators(booleanArrayList, queryParameter.getLogicalOperators());
			}
			if (result) {
				Row rowMap = new Row();
	...
	private boolean solveOperators(ArrayList<Boolean> booleanArrayList, List<String> operators) {
		if (booleanArrayList.size() == 1) {
			return booleanArrayList.get(0);
		} else if (booleanArrayList.size() == 2) {
			if (operators.get(0).matches("AND|and"))
				return booleanArrayList.get(0) & booleanArrayList.get(1);
			else
				return booleanArrayList.get(0) | booleanArrayList.get(1);
		} else if (booleanArrayList.size() == 3) {
			int i = operators.indexOf("AND|and");
			boolean res;
			if (i < 0)
				return booleanArrayList.get(0) | booleanArrayList.get(1) | booleanArrayList.get(2);
			else if (i == 0)
				return booleanArrayList.get(0) & booleanArrayList.get(1) | booleanArrayList.get(2);
			else if (i == 1)
				return booleanArrayList.get(0) | booleanArrayList.get(1) & booleanArrayList.get(2);
